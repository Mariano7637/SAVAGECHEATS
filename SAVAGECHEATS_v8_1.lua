--[[
    ╔═══════════════════════════════════════════════════════════════╗
    ║              SAVAGECHEATS_ AIMBOT UNIVERSAL v8.1              ║
    ║           UI PRÓPRIA + RAPID FIRE + SPEED BYPASS              ║
    ╠═══════════════════════════════════════════════════════════════╣
    ║  • UI 100% própria (não depende de libs externas)             ║
    ║  • Rapid Fire para Prison Life                                ║
    ║  • Speed Bypass melhorado (Property Spoofing)                 ║
...
if _G.SAVAGE_V81 then
    pcall(function() _G.SAVAGE_V81_CLEANUP() end)
    task.wait(0.3)
end
...
local Config = {
    ...
    CFrameMultiplier = 0.2,
    
    -- Speed (Spoof) - Mais arriscado mas mais rápido
    SpoofSpeedEnabled = false,
    SpoofSpeedValue = 25,
...
-- ═══════════════════════════════════════════════════════════════
--          SPEED SPOOF (PROPERTY SPOOFING - AVANÇADO)
-- ═══════════════════════════════════════════════════════════════

local SpoofSpeedActive = false
local SpoofedInstances = {}
local OldIdx, OldNewIdx, OldNC

local function EnableSpoofSpeed()
    if SpoofSpeedActive then return end
    
    -- Verificar se executor suporta as funções necessárias
    if not hookmetamethod or not hookfunction or not checkcaller then
        warn("[SAVAGECHEATS] Seu executor não suporta Speed Spoof!")
        warn("[SAVAGECHEATS] Use CFrame Speed ao invés disso.")
        return false
    end
    
    pcall(function()
        local Clone = game.Clone
        local Methods = {"FindFirstChild", "FindFirstChildOfClass", "FindFirstChildWhichIsA"}
        
        local function SpoofProp(Instance, Property)
            local Cloned = Clone(Instance)
            table.insert(SpoofedInstances, {
                Instance = Instance,
                Property = Property,
        ...
        OldIdx = hookmetamethod(game, "__index", function(self, key)
            for _, v in next, SpoofedInstances do
                if self == v.Instance and key == v.Property and not checkcaller() then
                    return OldIdx(v.ClonedInstance, key)
                end
                if key == "Parent" and (self == v.ClonedInstance or self == v.Instance) and not checkcaller() then
        ...
        OldNC = hookmetamethod(game, "__namecall", function(self, ...)
            ...
            if not table.find(Methods, Method) or not LocalPlayer.Character or self ~= LocalPlayer.Character then
                return OldNC(self, ...)
            end
            
            local Results = OldNC(self, ...)
            if Results and Results:IsA("Humanoid") and LocalPlayer.Character and self == LocalPlayer.Character then
                for _, v in next, SpoofedInstances do
        ...
        for i, Method in next, Methods do
            ...
            Old = hookfunction(game[Method], function(self, ...)
                if not LocalPlayer.Character or self ~= LocalPlayer.Character then
                    return Old(self, ...)
                end
        ...
        -- Aplicar spoof no Humanoid
        local char = LocalPlayer.Character
        if char then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum then
                SpoofProp(hum, "WalkSpeed")
                hum.WalkSpeed = Config.SpoofSpeedValue
                
                hum:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
                    if Config.SpoofSpeedEnabled then
                        hum.WalkSpeed = Config.SpoofSpeedValue
                    end
                end)
            end
        ...
        SpoofSpeedActive = true
    end)
    
    return SpoofSpeedActive
end

local function DisableSpoofSpeed()
    if not SpoofSpeedActive then return end
    ...
    SpoofSpeedActive = false
    SpoofedInstances = {}
end

-- ═══════════════════════════════════════════════════════════════
--                    RAPID FIRE (PRISON LIFE)
-- ═══════════════════════════════════════════════════════════════

local RapidFireApplied = {}

local function ApplyRapidFire(gunName)
    if RapidFireApplied[gunName] then return true end
    
    local success = pcall(function()
        local gun = LocalPlayer.Backpack:FindFirstChild(gunName) or 
                    (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild(gunName))
...
local function EnableRapidFire()
    -- Lista de armas do Prison Life
    local guns = {"Remington 870", "AK-47", "M9", "M4A1"}
    
    for _, gunName in ipairs(guns) do
        ApplyRapidFire(gunName)
    end
    
    -- Monitorar novas armas
    if not Connections.RapidFireBackpack then
        Connections.RapidFireBackpack = LocalPlayer.Backpack.ChildAdded:Connect(function(child)
            if Config.RapidFireEnabled then
                task.wait(0.1)
                ApplyRapidFire(child.Name)
            end
        end)
    end
    
    if LocalPlayer.Character then
        if not Connections.RapidFireCharacter then
            Connections.RapidFireCharacter = LocalPlayer.Character.ChildAdded:Connect(function(child)
                if Config.RapidFireEnabled then
                    task.wait(0.1)
                    ApplyRapidFire(child.Name)
                end
...
local function DisableRapidFire()
    ...
    if Connections.RapidFireBackpack then
        Connections.RapidFireBackpack:Disconnect()
        Connections.RapidFireBackpack = nil
    end
    
    if Connections.RapidFireCharacter then
        Connections.RapidFireCharacter:Disconnect()
        Connections.RapidFireCharacter = nil
    end
end
...
local function CreateWarning(parent, text)
...
local function CreateUI()
    ...
    ScreenGui.Name = "SAVAGECHEATS_V81"
    ...
    -- Seção Rapid Fire
    CreateSection(miscContent, "Rapid Fire")
    
    if IsPrisonLife then
        CreateWarning(miscContent, "⚡ Prison Life: Rapid Fire disponível!")
    end
    
    CreateToggle(miscContent, "Rapid Fire", false, function(v)
        Config.RapidFireEnabled = v
        if v then EnableRapidFire() else DisableRapidFire() end
    end)
    
    CreateSlider(miscContent, "Fire Rate (ms)", 1, 20, 5, function(v)
        Config.RapidFireRate = v / 100
    end)
    
    -- Seção Speed
    CreateSection(miscContent, "Speed")
    
    CreateWarning(miscContent, "⚠ CFrame Speed: Mais seguro!\n⚠ Spoof Speed: Mais rápido mas arriscado")
    
    CreateToggle(miscContent, "CFrame Speed", false, function(v)
        Config.CFrameSpeedEnabled = v
        if v then 
            Config.SpoofSpeedEnabled = false
            DisableSpoofSpeed()
            EnableCFrameSpeed() 
        else 
            DisableCFrameSpeed() 
    ...
    CreateSlider(miscContent, "CFrame Mult (÷10)", 1, 10, 2, function(v)
        Config.CFrameMultiplier = v / 10
    end)
    
    CreateToggle(miscContent, "Spoof Speed (Avançado)", false, function(v)
        Config.SpoofSpeedEnabled = v
        if v then
            Config.CFrameSpeedEnabled = false
            DisableCFrameSpeed()
            local success = EnableSpoofSpeed()
            if not success then
                Config.SpoofSpeedEnabled = false
            end
        else
    ...
    CreateSlider(miscContent, "Spoof Value", 16, 50, 25, function(v)
        Config.SpoofSpeedValue = v
        if Config.SpoofSpeedEnabled then
            local char = LocalPlayer.Character
            if char then
                local hum = char:FindFirstChildOfClass("Humanoid")
    ...
    infoBox.Size = UDim2.new(1, -10, 0, 200)
    infoBox.BackgroundColor3 = Theme.Surface
    infoBox.Text = [[SAVAGECHEATS v8.1
Aimbot Universal
...
• 2 tipos de Speed:
  - CFrame (seguro)
  - Spoof (avançado)
• UI própria otimizada
• Compatível com Mobile

Dicas:
• Use CFrame Speed 1-3 para segurança
• Spoof Speed requer executor avançado
• Rapid Fire funciona em todas armas]]
...
_G.SAVAGE_V81 = true
_G.SAVAGE_V81_CLEANUP = DestroyAll
...
local function Initialize()
    print("═══════════════════════════════════════════════════")
    print("       SAVAGECHEATS_ AIMBOT UNIVERSAL v8.1")
    print("       + RAPID FIRE + SPEED BYPASS")
    print("═══════════════════════════════════════════════════")
    print("Jogo: " .. GameName)
    
    if IsPrisonLife then
        print("⚡ Prison Life detectado!")
        print("  • Rapid Fire disponível")
        print("  • Use CFrame Speed 1-3 para segurança")
    end
    ...
    LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        -- Reaplicar Spoof Speed após respawn
        if Config.SpoofSpeedEnabled then
            SpoofSpeedActive = false
            SpoofedInstances = {}
            EnableSpoofSpeed()
